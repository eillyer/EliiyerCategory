...
- (NSString *)firstPinyinLetter
{
    ...
    int character = [self characterAtIndex:0];
    ...
    {		
		...
		{
			return [NSString stringWithFormat:@"%c", tolower(character)];
		}
		else
		{
			return @"#";
...
- (NSString *)firstPinyinLettersOfEachWord
{
    ...
    NSMutableString *pinyin = [[NSMutableString alloc] initWithCapacity:[self length]];
    ...
    for (int i = 0; i < [self length]; i++)
    {
        ...
        if (index >= 0 && index <= HANZI_COUNT)
        {
            [pinyin appendFormat:@"%c", firstLetterArray[index]];
        }
        else
        {
			if (isupper(character))
			{
				[pinyin appendFormat:@"%c", tolower(character)];
			}
			else
			{
				[pinyin appendFormat:@"%c", character];
			}
        }
    }
